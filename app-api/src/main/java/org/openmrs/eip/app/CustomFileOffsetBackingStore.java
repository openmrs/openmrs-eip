...
public class CustomFileOffsetBackingStore extends FileOffsetBackingStore {
	...
	public synchronized void start() {
		...
		try {
			//The offset file structure changed from that generated by pre 1.6 versions therefore, we need to transform any
			//existing offset file to match the new structure otherwise remote sites will lose any events that are recorded
			//between pre-upgrade and post-upgrade application runs of the sender.
