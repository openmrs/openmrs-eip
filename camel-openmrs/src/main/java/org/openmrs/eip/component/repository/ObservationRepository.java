...
public interface ObservationRepository extends SyncEntityRepository<Observation>, ObservationRepositoryCustom {
     ...
     */
    @Query(value = "SELECT count(o.obs_id) > 0 FROM obs o " +
            ...
            "WHERE CONCAT(crs.name,':',crt.code) = :conceptMapping " +
            "AND cmt.name = 'SAME-AS'" +
            "AND o.uuid = :uuid",
            nativeQuery = true)
    int isObsLinkedToGivenConceptMappingMySQL(@Param("uuid") String uuid,
                                              @Param("conceptMapping") String conceptMapping);
