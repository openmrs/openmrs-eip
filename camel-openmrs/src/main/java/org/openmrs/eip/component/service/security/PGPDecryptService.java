...
public class PGPDecryptService extends AbstractSecurityService implements Processor {
	...
	/**
	 * Verifies the signature and decrypts the message in parameter with the public key corresponding to
	 * the given userId
	 * 
	...
	public String verifyAndDecrypt(final String encryptedMessage) {
		...
		try (BufferedOutputStream bufferedOutputStream = new BufferedOutputStream(unencryptedOutputStream);
		        InputStream bouncyGPGInputStream = BouncyGPG.decryptAndVerifyStream().withConfig(keyRing)
	...
	/**
	 * Verifies and decrypts the message and puts it in the body with the senders public key userId from
	 * the header to know with which public key to decrypt the message
	 * 
