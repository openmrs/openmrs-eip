...
class TokenCacheTest {
	...
	void shouldFetchNewTokenWhenWithinClockSkewWindow() {
		...
		TokenInfo oldToken=new TokenInfo();oldToken.setExpiresIn("3600");oldToken.setAccessToken("oldToken");oldToken.setTokenType("Bearer");
		
		// Set expiry time to 20 seconds from now (within clock skew window)
		long expiryTime=System.currentTimeMillis()+20_000;tokenCache.setTokenInfo(oldToken);tokenCache.setExpiryTime(expiryTime);
		
		TokenInfo newToken=new TokenInfo();newToken.setExpiresIn("3600");newToken.setAccessToken("newToken");newToken.setTokenType("Bearer");
		...
		verify(openmrsFhirOauth2).fetchTokenInfo();verify(openmrsFhirOauth2).getClockSkewSeconds();
	}
}
