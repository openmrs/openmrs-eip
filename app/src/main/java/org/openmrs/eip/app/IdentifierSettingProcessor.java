 ...
 * In case of a subclass table, this processor retrieves the uuid from it's parent table e.g person.uuid value for
 * patient table
 */
...
public class IdentifierSettingProcessor implements Processor {
    ...
    public void process(Exchange exchange) {
        ...
        if ("d".equals(event.getOperation())) {
            uuid = event.getPreviousState().get(OpenmrsEipConstants.FIELD_UUID);
        } else {
            uuid = event.getCurrentState().get(OpenmrsEipConstants.FIELD_UUID);
        }
        ...
        if (uuid != null) {
            ...
            String query = "SELECT uuid FROM " + refTable + " WHERE " + refColumn + "=" + event.getPrimaryKeyId() +
                    "?dataSource=" + OPENMRS_DATASOURCE_NAME;
            List<Map> rows = DefaultFluentProducerTemplate.on(exchange.getContext()).to("sql:" + query).request(List.class);
            event.setIdentifier(rows.get(0).get(OpenmrsEipConstants.FIELD_UUID).toString());
        }
